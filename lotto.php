		...
		<div class="row">
			...
			<div class="col-md-9">
				<article>
					...
					<input type="button" value="click" onclick="button1_click()" class="btn btn-default">
					<p id="lotto_target"></p>
					<script>
					function button1_click() {var lotto = {
						'a':Math.floor((Math.random()*45)+1),
						'b':Math.floor((Math.random()*45)+1),
						'c':Math.floor((Math.random()*45)+1),
						'd':Math.floor((Math.random()*45)+1),
						'e':Math.floor((Math.random()*45)+1),
						'f':Math.floor((Math.random()*45)+1),
					}
					...
					if(lotto.a === lotto.b ||
						...
						lotto.e === 0 ||
						lotto.f === 0){
							document.getElementById("lotto_target").innerHTML = "중복발생"
						} else {
							for(key in lotto){
								document.getElementById("lotto_target").innerHTML = lotto[key];
								}
						}
					}
					</script>
